...
function AcadmicForFillOrNot($fromDate,$ToDate){
    include('../database_connection.php');
    $query = "SELECT checklist_record.checklist_id,checklist_record.class_date,checklist_record.batch,checklist_record.subject,checklist_record.faculty,checklist_record.batch_coordinator,checklist_record.venue,academic_feedback_record.class_start_time,academic_feedback_record.overall_rating_for_the_class 
    ...
    while($data = mysqli_fetch_assoc($run)){
        ...
        "faculty" => $data['faculty'],"batch_coordinator" => $data['batch_coordinator'],"venue"=>$data['venue'],"class_start_time" => $data['class_start_time'],'overall_rating_for_the_class'=>$data['overall_rating_for_the_class']
    ); 
...
function fetchDataForMasterSheet($fromDate){
    include("../database_connection.php");

    $query = "SELECT checklist_record.checklist_id,checklist_record.class_date,checklist_record.class_id_from_lecture_list,checklist_record.batch,checklist_record.subject,checklist_record.faculty,
    checklist_record.batch_coordinator,attendance_assignment_record.attendance,
    attendance_assignment_record.response,academic_feedback_record.class_start_time,academic_feedback_record.factual_errors_or_conceptual_lags,academic_feedback_record.factual_errors_or_conceptual_lags_text,academic_feedback_record.pace_of_the_class,
    academic_feedback_record.assignment_confirmed_with_faculty,academic_feedback_record.major_topics_covered,academic_feedback_record.management_technical_issue,academic_feedback_record.specific_issue_highlighted_by_students,academic_feedback_record.any_other_feedback,
    academic_feedback_record.No_of_live_query,academic_feedback_record.overall_rating_for_the_class,
    academic_feedback_record.video_portion_removed,
    academic_feedback_record_remark.video_portion_need_to_cut_remark
     FROM checklist_record LEFT JOIN attendance_assignment_record ON checklist_record.checklist_id = attendance_assignment_record.checklist_id
      LEFT JOIN academic_feedback_record ON checklist_record.checklist_id = academic_feedback_record.checklist_id
       LEFT JOIN academic_feedback_record_remark ON checklist_record.checklist_id = academic_feedback_record_remark.checklist_id
         WHERE (checklist_record.checklist_type = 'Class' OR checklist_record.checklist_type = 'Offline') AND checklist_record.class_date = '$fromDate'";
         $run = mysqli_query($connect,$query);
         while($data = mysqli_fetch_assoc($run)){

            $mastersheetArray[] = array('checklist_id'=>$data['checklist_id'],'class_date'=>$data['class_date'],'class_id_from_lecture_list'=>$data['class_id_from_lecture_list'],
        'batch'=>$data['batch'],'subject'=>$data['subject'],'faculty'=>$data['faculty'],'batch_coordinator'=>$data['batch_coordinator'],
        'attendance'=>$data['attendance'],'response'=>$data['response'],'class_start_time'=>$data['class_start_time'],'factual_errors_or_conceptual_lags'=>$data['factual_errors_or_conceptual_lags'],
        'factual_errors_or_conceptual_lags_text'=>$data['factual_errors_or_conceptual_lags_text'],'pace_of_the_class'=>$data['pace_of_the_class'],
        'assignment_confirmed_with_faculty'=>$data['assignment_confirmed_with_faculty'],'major_topics_covered'=>$data['major_topics_covered'],
        'No_of_live_query'=>$data['No_of_live_query'],'overall_rating_for_the_class'=>$data['overall_rating_for_the_class'],'management_technical_issue'=>$data['management_technical_issue'],'specific_issue_highlighted_by_students'=>$data['specific_issue_highlighted_by_students'],'any_other_feedback'=>$data['any_other_feedback'],
        'video_portion_removed'=>$data['video_portion_removed'],'video_portion_need_to_cut_remark'=>$data['video_portion_need_to_cut_remark']);

         }
return $mastersheetArray;
         mysqli_close($connect); 
