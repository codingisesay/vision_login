...
function fetch_complaint_data_from_to_date($from_date,$to_date){
    include('../database_connection.php');
    $query = "SELECT complaint_record.student_reg_no,complaint_record.complain_text,mode_of_complaint.mode_name,
     complaint_record.complaint_received, complaint_category.complaint_category_name,
      ...
      complaint_status.complaint_status_name,complaint_record.current_date,user.user_name FROM `complaint_record` 
      ...
      LEFT JOIN user ON complaint_record.updated_by = user.user_id WHERE current_date between '$from_date' and '$to_date'";
