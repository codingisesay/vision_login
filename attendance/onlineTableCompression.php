    ...
    <div class="panel panel-default">
      <div class="panel-heading">
        <h4 class="panel-title">
          <a data-toggle="collapse" data-parent="#accordion" href="#collapse<?php echo $i; ?>"><?php $runForBatchDel = batchDetail($batchSep); $dataForBatchDetail = mysqli_fetch_assoc($runForBatchDel); echo $dataForBatchDetail['batch_short_name']." ( Total Strength : ".$dataForBatchDetail['online_student']." ) ";?></a>
        </h4>
      </div>
...
while($data = mysqli_fetch_assoc($runForSelectedBatchClass)){
    $batch = $data['batch'];
    $onlineattendence = $data['response'];
    if(strpos($batch,',')){
       ...
       for($j = 0; $j < $batchCombineCount;$j++){
           ...
           $totalCombStudent = $totalCombStudent+$dataForTotalStu['online_student'];
           if($batchInArray[$i] == $batchCombine[$j]){
               ...
               $totalStudentofSelectedBatch = $datatotalStudentofSelectedBatch['online_student'];
           }
       }
      $totalCombStudent;
      $onlineattendence = $data['response']; //Present Student
      ...
      $noOfStudentInSelectedBatch = round($onlineattendence*$percentageOfselectedbatch/100); //Prob. Student
      $attend = round($noOfStudentInSelectedBatch*100/$totalStudentofSelectedBatch);

    }else{

       $onlineattendence = $data['response']; //Present Student	
       $noOfStudentInSelectedBatch = $onlineattendence;//Prob. Student
       $runForBatchAtt = batchDetail($batch);
       $dataForBatchAtt = mysqli_fetch_assoc($runForBatchAtt);
       $totalCombStudent = $dataForBatchAtt['online_student']; //Total Student
       $totalStudentofSelectedBatch = $totalCombStudent; //Selected Batch	
       $percentageOfselectedbatch = "100";//% Selected Batch
    //    $attend = round($onlineattendence*100/$totalCombStudent);
    //    // $attend = intval($t);
        ...
        <td><?php echo $onlineattendence;?></td>
