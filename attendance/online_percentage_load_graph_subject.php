...
$offlineStudentForSelBat = $dataForSelectedBatch['online_student'];//total student in selected batch
...
<?php
  ...
  for($graph = 0; $graph < $all_class_unique_reindex_count; $graph++){
    ...
    for($i = 0; $i < $dataForSubectWiseCount; $i++){
       ...
       $percentageOfSelectedBatchAtt = round($probabilityNoOfStudent*100/$offlineStudentForSelBat);
      ...
      "attendance" => $dataForSubectWise[$i]['attendance'],"response" => $dataForSubectWise[$i]['response'],"total student in CB" => $totalStudentInComBatch,
  ...
  for($inputDate = 0; $inputDate < $datesCount; $inputDate++){
    ...
    for($datasubwise = 0; $datasubwise < $newAllClassInBatchcount; $datasubwise++ ){
        ...
        if($datesArray[$inputDate] == $newAllClassInBatch[$datasubwise]['class_date']){?>
           ...
           { label: "<?php   $orgDate = "$datesArray[$inputDate]";  echo $newDate = date("d/m", strtotime($orgDate));   ?>",  y: <?php echo $newAllClassInBatch[$datasubwise]['percentageOfSelectedBatchAtt']; ?>, indexLabel : "<?php echo $newAllClassInBatch[$datasubwise]['percentageOfSelectedBatchAtt']."%"; ?>"},
