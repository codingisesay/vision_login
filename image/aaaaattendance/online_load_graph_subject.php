  ...
  for($inputDate = 0; $inputDate < $datesCount; $inputDate++){
    ...
    for($datasubwise = 0; $datasubwise < $newAllClassInBatchcount; $datasubwise++ ){
        ...
        if($datesArray[$inputDate] == $newAllClassInBatch[$datasubwise]['class_date']){?>
           ...
           { label: "<?php   $orgDate = "$datesArray[$inputDate]";  echo $newDate = date("d/m", strtotime($orgDate));   ?>",  y: <?php echo $newAllClassInBatch[$datasubwise]['response']; ?>, indexLabel : "<?php echo $newAllClassInBatch[$datasubwise]['response']; ?>"},
